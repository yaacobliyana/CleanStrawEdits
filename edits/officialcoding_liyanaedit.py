...
label=tkinter.Label(window, font="Helvetica 14 bold",fg="purple")
...
def step1():
    ...
    if analogInput(4) <= 900 :
        ...
        if analogInput(5) >= 700:
            print("Heating element 1 open until reach 90 degree Celcius")
            GPIO.output(22,1) #Heating element 1 open
...
def step2():
    if analogInput(4) <= 900 :
        ...
        if analogInput(4) <= 900 :
            ...
            print("Pump open, Valve A, B open and C close, Heating element 2 open")
            time.sleep(40)
...
Button1 = tkinter.Button(window,text='START',command =threading, font="Times 20 bold", bg="green", fg='white').grid(row=100, column=204)
 
Button = tkinter.Button(window,text='END',command=window.destroy, font="Times 20 bold", bg="red", fg='white').grid(row=100, column=206)
window.mainloop()
